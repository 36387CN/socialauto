...
router.post("/register", async (req, res) => {
  try {
    ...
    run("INSERT INTO users (email, password, name, company, referral_code, referred_by, free_days, is_admin) VALUES (?, ?, ?, ?, ?, ?, ?, (SELECT CASE WHEN (SELECT COUNT(*) FROM users) = 0 THEN 1 ELSE 0 END))",
      [email, hashed, name || "", company || "", code, referredBy, freeDays]);
    saveDb();

    const rows = query("SELECT id, email, name, company, plan, referral_code, free_days, is_admin FROM users WHERE email = ?", [email]);
    const u = rows[0].values[0];
    req.session.user = {
      id: u[0], email: u[1], name: u[2], company: u[3],
      plan: u[4], referral_code: u[5], free_days: u[6], is_admin: u[7] || 0, is_admin: u[7]
    };
...
router.post("/login", async (req, res) => {
  try {
    ...
    const rows = query("SELECT id, email, password, name, company, plan, referral_code, free_days, is_admin FROM users WHERE email = ?", [email]);
    ...
    req.session.user = {
      ...
      plan: u[5], referral_code: u[6], free_days: u[7], is_admin: u[8]
    };
    res.redirect("/dashboard");
  } catch (err) {
    console.error(err);
